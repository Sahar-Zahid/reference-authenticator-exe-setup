...
# ============================================================
# BATCHED AI PARSER - PRIMARY PARSER
#
# FIX: this is now the PRIMARY parser for every reference, not a
# fallback for the ones that failed deterministic parsing. Reason:
# the deterministic parser could "succeed" (return a non-None
# ParsedReference) while still extracting the wrong title/authors/
# year for a genuinely valid reference - and that incorrect metadata
# then made a real, valid reference come out NOT VERIFIED further
# down the pipeline. Routing every reference through the AI parser
# first (grouped into ~8-per-request batches, mirroring
# ai_verify_batch()'s existing batching pattern and run_with_fallback
# infra via parser_chain) fixes that specific failure mode while still
# only costing one LLM request per ~8 references.
#
# parse_reference_deterministic() (above) is preserved and still used,
# in two ways:
#   1. As the input to make_batch_parse_task's prompt is NOT affected -
#      deterministic parsing is simply no longer the primary path.
#   2. As an in-process fallback utility inside parse_references_batch:
#      if the AI batch call fails entirely for a chunk (all providers
#      exhausted), each reference in that chunk is parsed
#      deterministically instead of being silently degraded; only if
#      that ALSO fails does a reference fall back to a best-effort raw
#      title (_best_effort_parsed_reference).
# ============================================================
...
def _parse_with_deterministic_fallback(reference):
    """
    Fallback utility used ONLY when the AI batch parser is completely
    unavailable for a chunk. Tries the existing deterministic parser
    first (still a fully valid parse when it works), and only degrades
    to a best-effort raw title if the deterministic parser also can't
    find a usable title. Keeps parse_reference_deterministic() alive in
    the pipeline as the fallback/utility the task calls for, instead of
    removing it.
    """
    try:
        det = parse_reference_deterministic(reference)
    except Exception:
        det = None
    return det if det is not None else _best_effort_parsed_reference(reference)


async def parse_references_batch(pending, batch_size=8):
    """
    `pending` is a list of {"reference_id": int, "reference": str} for
    ALL references to be parsed (this is now the primary parsing path -
    see the module comment above). Returns {reference_id: ParsedReference}.

    On any failure (a chunk's LLM call fails entirely, or the model
    returns fewer results than requested), each affected reference
    degrades individually: parse_reference_deterministic() is tried
    first as a fallback utility, and only if that also can't find a
    usable title does it fall back further to
    _best_effort_parsed_reference(). A reference is never lost, never
    left unparsed.
    """
    all_parsed = {}
    ...
    for start in range(0, len(pending), batch_size):
        ...
        try:
            ...
            print(f"  \u26a0 AI batch parser unavailable for this chunk ({e}) - "
                  f"falling back to the deterministic parser for each reference instead.", flush=True)
            for c in chunk:
                all_parsed[c["reference_id"]] = _parse_with_deterministic_fallback(c["reference"])
        ...
        for ref_id in missing_ids:
            ...
            all_parsed[ref_id] = _parse_with_deterministic_fallback(c["reference"])
...
async def run_batch_verification(references):
    ...
    print("\n" + "=" * 70)
    print("PHASE 0 - PARSING (batched AI parser, primary)")
    print("=" * 70)

    # FIX: the batched AI parser is now the PRIMARY parser for every
    # reference (previously the deterministic parser ran first and the
    # AI parser only saw references it failed on). Reason: the
    # deterministic parser could technically return a non-None
    # ParsedReference for a genuinely valid reference while still
    # extracting the wrong title/authors/year - that incorrect metadata
    # then caused real, valid references to come out NOT VERIFIED
    # further down the pipeline. References are grouped into
    # ~8-per-request batches (batch 1 = refs 1-8, batch 2 = refs 9-16,
    # etc.) exactly like ai_verify_batch()'s existing batching pattern,
    # so this doesn't multiply LLM calls. parse_reference_deterministic()
    # is preserved and still used - as a fallback utility inside
    # parse_references_batch() when the AI parser is completely
    # unavailable for a batch (see cell 16 for details).
    ai_pending = [{"reference_id": ref_id, "reference": reference}
                  for ref_id, reference in enumerate(references, 1)]

    print(f"  \u2192 Batch AI parsing {len(ai_pending)} reference(s)...", flush=True)
    parsed_map = await parse_references_batch(ai_pending, batch_size=8)
    print(f"  \u2713 {len(parsed_map)}/{len(references)} reference(s) parsed.", flush=True)
    ...
    # ========================================================
    # EXPLANATION ENRICHMENT (purely additive)
    #
    # Exposes WHY each result received its status using data the
    # pipeline already produced (states/scores/candidates/notes) -
    # never changes `status` and never invents a new judgment. See
    # _enrich_result_explanation() below.
    # ========================================================

    for result in phase1_results:
        _enrich_result_explanation(result)
...
# ============================================================
# EXPLANATION ENRICHMENT HELPERS
#
# ADDITIVE ONLY: these expose already-computed comparison/search
# information (states, candidate scores, notes, metadata_conflicts) as
# clearly-named fields on each result, so a NOT VERIFIED/UNCERTAIN
# result's cause is legible (title mismatch vs. author mismatch vs. no
# similar record vs. insufficient metadata, etc.) without changing the
# underlying VERIFIED/UNCERTAIN/NOT VERIFIED decision logic anywhere.
# ============================================================

def _derive_reason_category(result):
    """
    Returns a short, specific WHY-label derived ONLY from data the
    verification system already produced for this result - never a
    guessed/invented reason.
    """
    status = result.get("status", "UNKNOWN")
    states = result.get("states") or {}
    notes = result.get("notes") or []
    ranked = result.get("ranked") or []
    metadata_conflicts = result.get("metadata_conflicts") or []

    if status == "VERIFIED":
        return "verified match"

    if not ranked:
        return "no sufficiently similar database record found"

    if states.get("doi") == "MISMATCH":
        return "DOI mismatch"
    if states.get("title") == "MISMATCH":
        return "title mismatch"
    if states.get("authors") == "MISMATCH":
        return "author mismatch"
    if states.get("year") == "MISMATCH":
        return "year mismatch"

    if metadata_conflicts:
        return "conflicting database results (" + "; ".join(str(m) for m in metadata_conflicts) + ")"

    if states.get("doi") == "MISSING_DATABASE" and status == "UNCERTAIN":
        return "DOI could not be confirmed against database record"

    if notes:
        return "insufficient metadata (" + "; ".join(notes) + ")"

    if status == "UNCERTAIN" and result.get("margin") is not None:
        return "conflicting/ambiguous database results (close-scoring candidates)"

    if result.get("source") == "AI verification":
        return "manual check required - AI verification found insufficient/ambiguous evidence"

    return "manual check required"


def _enrich_result_explanation(result):
    """
    Adds clearly-named explanation/evidence fields to a result IN
    PLACE. Does not touch `status` or any classification field - only
    exposes information (states, candidate scores, parsed/matched
    metadata) that the pipeline already computed earlier.
    """
    parsed = result.get("parsed_reference", {}) or {}
    best = result.get("best_match") or {}
    candidate = best.get("candidate", {}) if best else {}
    status = result.get("status", "UNKNOWN")

    result["verification_reason"] = result.get("explanation", "")
    result["reason_category"] = _derive_reason_category(result)

    result["submitted_title"] = parsed.get("title")
    result["matched_title"] = candidate.get("title")
    result["title_similarity"] = best.get("title_score")

    result["submitted_authors"] = parsed.get("authors", []) or []
    result["matched_authors"] = candidate.get("authors", []) or []
    result["author_similarity"] = best.get("author_score")

    result["submitted_year"] = parsed.get("year")
    result["matched_year"] = candidate.get("year")

    result["submitted_doi"] = parsed.get("doi")
    result["matched_doi"] = candidate.get("doi")
    ...
    sources_checked = sorted({
        ...
        for s in (
            ranked_item["candidate"].get("sources")
            or ([ranked_item["candidate"].get("source")] if ranked_item["candidate"].get("source") else [])
        )
    })
    result["sources_checked"] = sources_checked
    # `sources_agreeing` is already populated earlier in the pipeline.

    result["verification_source"] = result.get("source", "")
    result["manual_check_needed"] = status in ("UNCERTAIN", "NOT VERIFIED")
...
async def process_reference_phase1(reference, ref_id, parsed_reference):
    ...
    result = {
        ...
        # ADDITIVE: kept so later explanation-enrichment can report
        # exactly which fields matched/mismatched (see
        # _enrich_result_explanation in the controller cell) without
        # recomputing anything. Never read by any classification logic.
...
# --------------------------------------------------
# Column layout for the exported workbook.
#
# FIX (Excel formatting improvement): this replaces the previous plain
# `pd.DataFrame(rows).to_excel(...)` call with the SAME underlying data
# (plus the new additive explanation fields from
# _enrich_result_explanation - see the controller cell) laid out with a
# frozen header row, autofilter, sensible per-column widths, and
# wrapping only on genuinely long free-text columns. Every value below
# is read from the result dict exactly as already produced elsewhere in
# the pipeline - nothing here changes a verification decision.
#
# Each tuple is: (header, getter(result) -> value, column width, wrap_text)
# --------------------------------------------------
def _get_submitted(r, field):
    """Falls back to parsed_reference[field] if the enrichment step
    (_enrich_result_explanation) hasn't run for this result for any
    reason - keeps export_results_to_excel robust on its own."""
    value = r.get(f"submitted_{field}")
    if value not in (None, [], ""):
        return value
    return (r.get("parsed_reference", {}) or {}).get(field)


_EXCEL_COLUMNS = [
    ("Ref #",               lambda r: r.get("reference_id"),                                              7,  False),
    ("Status",               lambda r: r.get("status", "UNKNOWN"),                                          14, False),
    ("Manual Check Needed",  lambda r: "Yes" if r.get("manual_check_needed", r.get("status") in ("UNCERTAIN", "NOT VERIFIED")) else "No", 12, False),
    ("Reason Category",      lambda r: r.get("reason_category", ""),                                        28, True),
    ("Explanation",          lambda r: r.get("verification_reason", r.get("explanation", "")),               50, True),
    ("Original Reference",   lambda r: r.get("original_reference", ""),                                      55, True),
    ("Submitted Title",      lambda r: _get_submitted(r, "title") or "",                                     35, True),
    ("Matched Title",        lambda r: r.get("matched_title") or "",                                         35, True),
    ("Title Similarity",     lambda r: r.get("title_similarity"),                                            11, False),
    ("Submitted Authors",    lambda r: ", ".join(_get_submitted(r, "authors") or []),                        30, True),
    ("Matched Authors",      lambda r: ", ".join(r.get("matched_authors") or []),                             30, True),
    ("Author Similarity",    lambda r: r.get("author_similarity"),                                            11, False),
    ("Submitted Year",       lambda r: _get_submitted(r, "year") or "",                                        9, False),
    ("Matched Year",         lambda r: r.get("matched_year") or "",                                            9, False),
    ("Submitted DOI",        lambda r: _get_submitted(r, "doi") or "",                                        28, False),
    ("Matched DOI",          lambda r: r.get("matched_doi") or "",                                            28, False),
    ("Sources Checked",      lambda r: ", ".join(r.get("sources_checked") or []),                             24, True),
    ("Sources Agreeing",     lambda r: ", ".join(r.get("sources_agreeing", []) or []),                        20, True),
    ("Verification Source",  lambda r: r.get("verification_source", r.get("source", "")),                     20, False),
]

_HEADER_FILL = PatternFill(start_color="2F5597", end_color="2F5597", fill_type="solid")
_HEADER_FONT = Font(bold=True, color="FFFFFF")
_CHARS_PER_LINE_FACTOR = 1.8  # rough chars-that-fit per unit of Excel column width
_MIN_ROW_HEIGHT = 15
_MAX_ROW_HEIGHT = 150
...
def export_results_to_excel(results, filename="reference_verification_results.xlsx"):
    """
    Saves all results to a clean, professional, editable spreadsheet -
    one row per reference, with the reference number for easy
    cross-checking against the original paper's numbered citation list.

    FIX: formatting-only improvement. Column set/order, header
    freeze/autofilter/styling, column widths, and selective text
    wrapping (long free text wraps; DOI/year/similarity scores stay on
    one line) are new. The underlying verification data - status,
    explanation, matched metadata, scores - is exactly what the rest of
    the pipeline already produced; nothing here changes a result.
    """
    rows = []
    for r in results:
        # Be robust even if a result somehow bypassed
        # _enrich_result_explanation (e.g. a hand-built early-error
        # result) - fill in the enrichment fields on the fly rather
        # than crashing the export.
        if "reason_category" not in r:
            try:
                _enrich_result_explanation(r)
            except Exception:
                pass
        rows.append({header: getter(r) for header, getter, _, _ in _EXCEL_COLUMNS})

    df = pd.DataFrame(rows)
    if "Ref #" in df.columns and not df.empty:
        df = df.sort_values("Ref #")

    sheet_name = "Verification Results"
    df.to_excel(filename, index=False, engine="openpyxl", sheet_name=sheet_name)

    # ---- Post-process the workbook: header styling, freeze, filter,
    # column widths, selective wrapping. ----
    wb = load_workbook(filename)
    ws = wb[sheet_name]

    for col_idx, (header, _, width, wrap) in enumerate(_EXCEL_COLUMNS, start=1):
        ...
        header_cell.alignment = Alignment(vertical="center", wrap_text=False)

        body_alignment = Alignment(wrap_text=True, vertical="top") if wrap else Alignment(vertical="top")
        for row_idx in range(2, ws.max_row + 1):
            ws.cell(row=row_idx, column=col_idx).alignment = body_alignment

    ws.freeze_panes = "A2"          # freeze header row
    ws.auto_filter.ref = ws.dimensions  # enable filters on every column
    ws.row_dimensions[1].height = 20

    # Give wrapped rows enough height to actually show their text
    # (Excel doesn't auto-size row height for wrap_text on its own),
    # without ballooning every row - only wrapped columns are measured,
    # and height is capped so one very long explanation can't make a
    # row absurdly tall.
    wrap_columns = [(idx, width) for idx, (_, _, width, wrap) in enumerate(_EXCEL_COLUMNS, start=1) if wrap]
    for row_idx in range(2, ws.max_row + 1):
        max_lines = 1
        for col_idx, width in wrap_columns:
            value = ws.cell(row=row_idx, column=col_idx).value
            if not value:
                continue
            chars_per_line = max(10, int(width * _CHARS_PER_LINE_FACTOR))
            lines = -(-len(str(value)) // chars_per_line)  # ceil division
            max_lines = max(max_lines, lines)
        ws.row_dimensions[row_idx].height = min(max(_MIN_ROW_HEIGHT * max_lines, _MIN_ROW_HEIGHT), _MAX_ROW_HEIGHT)
...
def generate_final_report(results):
    ...
    for i, result in enumerate(results, 1):
        ...
        # ADDITIVE: reason_category is the short WHY-label produced by
        # _enrich_result_explanation() (e.g. "title mismatch", "author
        # mismatch", "no sufficiently similar database record found").
        # Purely informational - doesn't replace or alter the full
        # explanation text on the next line.
        if result.get("reason_category"):
            lines.append(f"Reason category: {result['reason_category']}")
